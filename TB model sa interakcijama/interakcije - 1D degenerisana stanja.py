...
def construct_hamiltonian_from_operators(parameters=None):
    ...
    n_dim = len(fock_states)
    out = np.zeros((n_dim, n_dim))
    a = np.zeros((n_orb, n_dim, n_dim))
    c = np.zeros((n_orb, n_dim, n_dim))
    for i in range(n_orb):
    ...
    for i in range(n_orb):
        ...
        for d in [-1, 1]:
            j = (i + d) % n_orb  # nadji indeks suseda
            out += parameters['t'] * c[i] @ a[j]
        out += parameters['V'] * (c[i] @ a[i]) @ \
            (c[(i+1) % n_orb] @ a[(i+1) % n_orb])
    return out
...
def main(parameters):
    ...
    for i in range(n_orb):
        ...
        n2 = get_number_operator((i+1) % n_orb)
...
if __name__ == '__main__':
    parameters = {'t': -1, 'eps': -10, 'V': 10, 'n_orb': 4,
